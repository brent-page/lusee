...
def local_hour_from_azimuth_altitude(A_azimuth, h_altitude, phi_latitude):
    return np.arctan2( np.sin(A_azimuth) * np.cos(h_altitude) , np.cos(A_azimuth) * np.sin(phi_latitude) * np.cos(h_altitude) + np.sin(h_altitude) * np.cos(phi_latitude) )

def declination_from_azimuth_altitude(A_azimuth, h_altitude, phi_latitude):
    return np.arcsin(np.sin(phi_latitude) * np.sin(h_altitude) - np.cos(phi_latitude) * np.cos(h_altitude) * np.cos(A_azimuth))
...
def transformation_test():
    ...
    declination_method_2 = declination_from_azimuth_altitude(A_azimuth, h_altitude, SCHROD_LAT)
    ...
    right_ascension_method_2[idxs] -= np.sign(right_ascension_method_2[idxs]) * 2 * np.pi
...
def get_altitude(body, times):
    ...
    body_altitude = 90 - np.arccos(np.matmul(body_pos, SCHROD_OUTWARD_NORMAL)) * 180/np.pi
    return body_altitude

def get_sun_earth_from_horizon():
    utc_times = np.arange(datetime(2024, 1, 1), datetime(2024, 12, 31, 23, 59), timedelta(minutes = 15)).astype(datetime)
    ...
    start_both_below_idxs, end_both_below_idxs = get_set_rise_idxs(sun_earth_below_horizon)
    start_both_above_idxs, end_both_above_idxs = get_set_rise_idxs(sun_earth_above_horizon)
...
def plot_inclinations_from_horizon():
    utc_times = np.arange(datetime(2024, 1, 1), datetime(2025, 12, 31, 23, 59), timedelta(minutes = 15)).astype(datetime)
...
#     all_below_horizon = ((saturn_altitude < 0) & (jupiter_altitude < 0) & (sun_altitude < 0) & (neptune_altitude < 0) & (uranus_altitude < 0))
    sun_jupiter_saturn_below_horizon = ((saturn_altitude < 0) & (jupiter_altitude < 0) & (sun_altitude < 0))
...
#     ura_nep_below_horizon = ((uranus_altitude < 0) & (neptune_altitude < 0))
    ...
    for below_horizon, name, label in zip([sun_below_horizon, all_below_horizon, sun_jupiter_below_horizon, ura_nep_below_horizon], ['sun_radio_quiet_times', 'sun_jup_sat_ura_nep_radio_quiet_times', 'sun_jupiter_radio_quiet_times', 'uranus_neptune_radio_quiet_times'], ['sun', 'sun, jupiter, saturn, uranus, neptune', 'sun, jupiter', 'uranus, neptune']):
        ...
        to_export2 = xr.Dataset(
                {
                    label + " below horizon start": (['idx'], utc_times[radio_quiet_start_idxs]),
                    label + " below horizon end": (['idx'], utc_times[radio_quiet_end_idxs]),
                    "duration": (['idx'], durations)
                    }
                )
...
def print_lunar_night(year, month):
    utc_times = np.arange(datetime(year, month, 1) - timedelta(days = 20), datetime(year, month, 1) + timedelta(days = 50), timedelta(hours = 1)).astype(datetime)
    sun_altitude = get_altitude(SUN, utc_times)
...
#          pixels that are below the horizon can't be seen
    ...
    plt.title('RJ Temp, NS 20 MHz sigma: {:.0f}°, EW 20 MHz sigma: {:.0f}°'.format(np.arcsin(NS_20MHz_beam_stdev) * 180/np.pi, np.arcsin(EW_20MHz_beam_stdev) * 180/np.pi))
